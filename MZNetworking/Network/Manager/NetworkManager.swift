...
public protocol NetworkManagerProtocol {
    func request<E: JsonProvider>(_ request: E,completion: @escaping ((WebserviceResponseResult<E.Response>) -> Void))
    func download<E: Downloadable>(_ request: E, completion: @escaping ((WebserviceResponseResult<Data>) -> Void))
    func cancel(_ request: WebserviceRequestConvertible, completion: @escaping (Bool)->Void)
    func cancelAllPendingRequest()
}

typealias completionHandler = ((WebserviceResponseResult<Data>) -> Void)
...
public class NetworkManager : NetworkManagerProtocol {
    ...
    public func request<E: JsonProvider>(_ request: E,completion: @escaping ((WebserviceResponseResult<E.Response>) -> Void)) {
        ...
        if let urlString = urlRequest.url?.absoluteString,
            ...
            var result: WebserviceResponseResult<E.Response>!
    ...
    ///   - request: Generic Request Object based on Provider Type specified by the user.
    ///   - completion: Completion handler used for returning API Call Result.
    public func download<E>(_ request: E, completion: @escaping ((WebserviceResponseResult<Data>) -> Void)) where E : Downloadable {
        let endpoint = request.endpoint
        let numberOfRetry = endpoint.numberOfRetry
        ...
        if let urlString = urlRequest.url?.absoluteString,
            ...
            self.request(endpoint, request: urlRequest, numberOfRetry: numberOfRetry) { (response) in
                ...
                let result: WebserviceResponseResult<Data> = response
    ...
    private func request (_ api: WebserviceRequestConvertible, request: URLRequest, numberOfRetry: Int, completion: @escaping completionHandler) {
        ...
        if let dataTask = sessionStore.getDataTask(for: urlString) {
            ...
            let task = manager.dataTask(with: request) { (data, response, error) in
                ...
                let result: WebserviceResponseResult<Data>
    ...
    public func cancel(_ request: WebserviceRequestConvertible, completion: @escaping (Bool) -> Void) {
        
        guard let urlRequest = request.urlRequest() as URLRequest? else {
            completion(false)
            return
        }
