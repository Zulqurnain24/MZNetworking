...
final class URLSessionManager : NSObject, URLSessionDelegate {
    ...
    func set(_ dataTask: URLSessionDataTask?, for endPoint: WebserviceRequestConvertible, completion: @escaping completionHandler) {
    ...
    func removeRequest(for endPoint: WebserviceRequestConvertible) {
        queue.sync {
            if let url = endPoint.urlRequest().url?.absoluteString,
